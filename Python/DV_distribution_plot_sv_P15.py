...
#Objectif du script :
#Créer une liste de toutes les valeurs DV des variants structuraux pour P15 à partir des fichiers VCF sniffles de type SV
...
for filename in os.listdir(vcf_folder):
    if filename.endswith(".vcf") and filename.startswith("P15") and "sniffles" in filename:
...
output_file_support = os.path.join(list_dv_dir, "List_DV_P15.txt")
...
if not List_DV:
    print("Aucune valeur DV trouvee pour P15 avec ce dossier d'entree.")
    sys.exit(0)
...
plt.title("Distribution des valeurs DV des variants structuraux pour P15")
...
plot_path = os.path.join(plot_folder, "Distribution_DV_variants_SV_P15.png")
