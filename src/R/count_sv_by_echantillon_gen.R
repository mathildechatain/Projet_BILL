...
infile <- "../../Files/plot_files/sv_sc_counts.csv"
outfile <- "../../Plots/count_sv_by_echantillon_gen.png"
...
p <- ggplot(d, aes(x = generation, y = n_SV, group = replicate, color = factor(replicate))) +
  ...
  labs(
    title = "SV totaux: controle P15->P25 vs stress P25->P27 vs controle P30->P50",
    x = "Passage",
    y = "Nombre de SV",
    color = "Lignee"
  ) +
